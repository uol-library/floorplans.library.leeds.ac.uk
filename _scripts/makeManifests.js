...
featuresXML.forEach( filename => {
    if ( filename !== '.' && filename !== '..' ) {
        ...
        let width = parseInt(svgdata.svg['@_viewBox'].split(',')[2])+meta.adjust_x;
        let height = parseInt(svgdata.svg['@_viewBox'].split(',')[3])+meta.adjust_y;
        let manifest = {
                    ...
                    "annotations":  [
                        {
                            ...
                            "type": "AnnotationPage",
                            "items": getAnnotations( baseURI, svgdata, meta )
                        }
                    ]
                }
            ]
...
function getAnnotations( baseURI, svgdata, metadata ) {
    ...
    if (svgdata.svg.polygon.length) {
        ...
        let svgDir = path.resolve( __dirname, '../assets/iiif/', metadata.key, 'svg')
