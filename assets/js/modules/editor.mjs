...
export function initEditor() {
    ...
    L.Control.copyGeoJSON = L.Control.extend({
        ...
        onAdd: function(map) {
            ...
            L.DomEvent.on(floorplans.copyButton, 'click', function() {
                let geojson = floorplans.pmgroup.toGeoJSON();
    ...
    L.control.copygeojson().addTo(floorplans.map);
    ...
    L.Control.loadGeoJSON = L.Control.extend({
        ...
        onAdd: function(map) {
            ...
            floorplans.loadButton.setAttribute('title','Copy GeoJSON');
    ...
    /* add geoman controls */
    floorplans.pmgroup = L.layerGroup().addTo(floorplans.map);
    floorplans.map.pm.addControls({  
        position: 'topright',
    });
    floorplans.map.pm.setGlobalOptions({
        snappable: false,
        layerGroup: floorplans.pmgroup
    });
