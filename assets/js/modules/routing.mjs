...
function getFeatureFromClassmark( library, floorName, classmark ) {
    ...
    for ( let i = 0; i < floorplans.featureMap.length; i++ ) {
        let libraryRE = getLibraryRegex( library, floorName );
        if ( floorplans.featureMap[i][1].match( libraryRE ) ) {
            let featureRE = getFeatureRegex( floorplans.featureMap[i][0] );
            let featureDetails = false;
            if ( library === 'health-sciences' ) {
                if ( classmark === floorplans.featureMap[i][0] ) {
                    featureDetails = floorplans.featureMap[i][1].split( '-' );
                }
            ...
            if ( featureDetails ) {
                ret = {
                    "name": floorplans.featureMap[i][0],
